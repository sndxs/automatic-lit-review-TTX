...
**`report`** -- regenerates `deep_dive_report.tex`/`.pdf` from whatever's
currently in the pool: a "Findings by Country" section and a "Cross-Country
Themes" section. Only the theme-spotting is an LLM call (one call, output
size scales with the number of themes found, not the number of findings);
the bibliography and per-country findings list are built directly from the
structured DB rows in Python, so this can't hit the truncation failure
mode the original `paper_updater.py` design had (see git history) --
nothing here asks a model to reproduce data it was just given.

**`auto`** -- alternates `harvest`/`analyze` rounds for a time budget (e.g.
overnight), stopping early if harvesting is complete and the pool is fully
analyzed, then generates the report. Meant to be started as a detached
background process (see below) so it keeps running after you close the
terminal/app.
...
python deep_dive_news.py report                    # regenerate the LaTeX report from the pool
python deep_dive_news.py auto --hours 7             # alternate harvest/analyze for ~7 hours, then report
```

`deep_dive_news/` (the SQLite candidate pool + findings CSV) is gitignored
-- this is raw research data, not part of the paper. `deep_dive_report.tex`/
`.pdf` are committed, same as the literature review papers. Needs
...
To let `auto` run overnight independent of this terminal/app (Windows):
```powershell
Start-Process -FilePath "python" -ArgumentList "deep_dive_news.py auto --hours 7" `
  -WorkingDirectory "<project folder>" -WindowStyle Hidden
```
The machine needs to stay on (not asleep) for the duration, same
requirement as the daily scheduled task.
...
- `deep_dive_news/` -- output of the above: `candidates.db` and `findings.csv` (gitignored).
- `deep_dive_report.tex`/`.pdf` -- LaTeX report generated by `deep_dive_news.py report`, committed like the literature review papers.
- `latex_utils.py` -- shared LaTeX-escaping/Markdown-cleanup helpers used by `paper_updater.py` and `deep_dive_news.py`.
